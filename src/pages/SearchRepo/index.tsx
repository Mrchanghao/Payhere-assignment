...
export function SearchRepos() {
  ...
  const registerRepo = (repo: Repo) => {
    ...
    if (storeRepo.registeredRepo.length <= 4 && validRepo(repo, storeRepo) === false) {
      // 같은 repo 면 등록 안 되는 로직 구현 해야함
      setStoreRepo({
        registeredRepo: [...storeRepo.registeredRepo, {id, name, owner: login }],
      });
    } else {
      // modal 구현
  ...
  // if (repos.length === 0) return <h1>상단의 검색어를 입력하세요</h1>;
  // return (
  //   <PageWrapper>
